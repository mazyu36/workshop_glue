...
# S3 sink locations
output_path = args["s3_bucket"] + "/output/streaming/"
job_time_string = datetime.now().strftime("%Y%m%d%H%M%S")
s3_target = output_path + job_time_string
checkpoint_location = output_path + "checkpoint/"
temp_path = output_path + "temp/"

country_lookup_path = args["s3_bucket"] + "/input/streaming/country_lookup/"
...
def processBatch(data_frame, batchId):
    if data_frame.count() > 0:
        ...
        final_frame = Join.apply(
            apply_mapping, country_lookup_frame, "country", "CountryName"
        ).drop_fields(
            [
                "CountryName",
                "country",
